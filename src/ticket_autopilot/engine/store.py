...
Engine is intentionally stateless between runs — a workflow run is fully
...
def _runs_dir(engine_root: str) -> str:
    d = os.path.join(engine_root, "runs")
    os.makedirs(d, exist_ok=True)
    return d


def save(workflow_name: str, result: dict, params: dict, engine_root: str) -> str:
    path = os.path.join(_runs_dir(engine_root),
...
def list_runs(engine_root: str) -> list[str]:
    d = _runs_dir(engine_root)
