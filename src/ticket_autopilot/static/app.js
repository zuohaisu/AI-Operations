...
let activeOperationId = localStorage.getItem('ticket-autopilot.operation_id');
...
function renderProcessOutput(entries) { const wasNearBottom = processOutput.scrollHeight - processOutput.scrollTop - processOutput.clientHeight < 48; processOutput.textContent = (entries || []).map((entry) => `[${entry.timestamp}] ${entry.stage}/${entry.role} · ${entry.stream}\n${entry.message}`).join('\n\n') || 'Waiting for backend output…'; if (wasNearBottom) processOutput.scrollTop = processOutput.scrollHeight; }
...
async function loadTimeline() { if (!activeRunId) return; try { const payload = await json(`/api/runs/${encodeURIComponent(activeRunId)}`); const snapshot = payload.snapshot; timeline.hidden = false; setStatus(runStatus, snapshot.status === 'PASS' ? 'COMPLETED' : snapshot.status, ['BLOCKED', 'HARD_BREAK', 'TECHNICAL_BLOCKED', 'FAIL', 'QA_EXHAUSTED'].includes(snapshot.status)); activeWorktree = snapshot.worktree || snapshot.hard_break?.worktree || null; runSummary.textContent = `Run ${payload.run_id} · ${activeWorktree ? `worktree: ${activeWorktree}` : 'worktree evidence is retained on disk.'}`; renderProcessOutput(payload.process_output); eventsNode.replaceChildren(...payload.events.map(renderEvent)); renderHardBreak(snapshot); const active = ['ACTIVE', 'DEVELOPING', 'VERIFYING', 'QA_RUNNING'].includes(snapshot.status); document.querySelector('#retry').disabled = active || snapshot.status === 'STOPPED'; document.querySelector('#stop').disabled = !active; if (active && !pollTimer) pollTimer = setInterval(loadTimeline, 1200); if (!active && pollTimer) { clearInterval(pollTimer); pollTimer = null; } } catch (error) { setStatus(runStatus, error.message, true); if (pollTimer) { clearInterval(pollTimer); pollTimer = null; } } }
function beginTimeline(runId) { activeRunId = runId; activeOperationId = null; localStorage.setItem('ticket-autopilot.run_id', runId); localStorage.removeItem('ticket-autopilot.operation_id'); if (operationPollTimer) { clearInterval(operationPollTimer); operationPollTimer = null; } loadTimeline(); }
async function loadOperation() { if (!activeOperationId) return; try { const payload = await json(`/api/operations/${encodeURIComponent(activeOperationId)}`); timeline.hidden = false; setStatus(runStatus, payload.status, ['BLOCKED', 'BLOCKED_REQUIREMENTS', 'HARD_BREAK'].includes(payload.status)); runSummary.textContent = `Preparation ${payload.operation_id} · stage: ${payload.stage}`; renderProcessOutput(payload.process_output); eventsNode.replaceChildren(); hardBreak.hidden = true; document.querySelector('#retry').disabled = true; document.querySelector('#stop').disabled = true; if (payload.run_id) { beginTimeline(payload.run_id); return; } const active = ['PREPARING', 'STARTING'].includes(payload.status); if (active && !operationPollTimer) operationPollTimer = setInterval(loadOperation, 1200); if (!active && operationPollTimer) { clearInterval(operationPollTimer); operationPollTimer = null; } } catch (error) { setStatus(runStatus, error.message, true); if (operationPollTimer) { clearInterval(operationPollTimer); operationPollTimer = null; } } }
function beginOperation(operationId) { activeOperationId = operationId; localStorage.setItem('ticket-autopilot.operation_id', operationId); loadOperation(); }
async function loadLatestTimeline() { const payload = await json('/api/runs/latest'); if (payload.run_id) beginTimeline(payload.run_id); }
document.querySelector('#refresh').addEventListener('click', loadTickets);
document.querySelector('#prepare').addEventListener('click', async () => { if (!selected) return; const issueId = selected.id; setStatus(prepareResult, 'Preparing and saving prompts to tasks/…'); try { const result = await json(`/api/tickets/${encodeURIComponent(issueId)}/prepare`, {method: 'POST'}); if (result.status === 'READY') { await loadTickets(); await loadDetail(issueId); setStatus(prepareResult, `Saved for human use: ${result.prompts.dev.canonical_path} · ${result.prompts.acceptance.canonical_path}`); } else setStatus(prepareResult, `${result.status}: ${result.hard_break_reason || result.reason}`, true); } catch (error) { setStatus(prepareResult, error.message, true); } });
document.querySelector('#run').addEventListener('click', async () => { if (!selected) return; setStatus(prepareResult, 'Starting observable preparation…'); try { const result = await json(`/api/tickets/${encodeURIComponent(selected.id)}/run`, {method: 'POST'}); if (result.operation_id && result.status === 'PREPARING') { setStatus(prepareResult, `Preparation started: ${result.operation_id}`); beginOperation(result.operation_id); } else if (result.run_id) { setStatus(prepareResult, `Run started: ${result.run_id}`); beginTimeline(result.run_id); } else { setStatus(prepareResult, `${result.status}: ${result.reason || result.hard_break_reason}`, true); } } catch (error) { setStatus(prepareResult, error.message, true); } });
...
Promise.all([loadSettings(), loadTickets()]).then(() => activeOperationId ? loadOperation() : loadLatestTimeline()).catch((error) => { setStatus(ticketsStatus, error.message, true); });
