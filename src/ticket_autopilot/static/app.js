...
async function loadDetail(id) { try { selected = await json(`/api/tickets/${encodeURIComponent(id)}`); detail.hidden = false; setStatus(prepareResult, ''); detailBody.replaceChildren(); for (const [label, value] of [['Identifier', selected.identifier], ['State', selected.state.name || selected.state.group], ['Priority', selected.priority], ['Risk', selected.risk], ['Eligibility', selected.eligible ? 'Ready to prepare' : selected.reason], ['Prompt availability', promptSource(selected.prompt_availability)]]) addLine(detailBody, label, value); document.querySelector('#prepare').disabled = !selected.eligible; document.querySelector('#run').disabled = !selected.eligible; const ready = selected.prompt_availability; if (selected.eligible && ready.dev.available && ready.acceptance.available) setStatus(prepareResult, 'Prompts already exist in tasks/ — you can Run directly; Prepare is optional.'); } catch (error) { setStatus(ticketsStatus, error.message, true); } }
...
document.querySelector('#run').addEventListener('click', async () => { if (!selected) return; setStatus(prepareResult, 'Starting isolated Run…'); try { const result = await json(`/api/tickets/${encodeURIComponent(selected.id)}/run`, {method: 'POST'}); if (result.run_id) { setStatus(prepareResult, `Run started: ${result.run_id}`); beginTimeline(result.run_id); } else setStatus(prepareResult, `${result.status}: ${result.reason}`, true); } catch (error) { setStatus(prepareResult, error.message, true); } });
